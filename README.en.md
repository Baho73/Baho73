...
6 products in production (PlanPulse, Shtab, Fluffy Fox Ear, DefectMaster B2C and B2B, Belbin Role Test). 11 custom-built AI agents (pilots and live). 7 MCP servers of my own. Full cycle: from problem statement to a working corporate service or SaaS with billing.

How I work with AI-generated code: the module contract is written before the code; tests are written by a separate agent and kept hidden from the agent that writes the implementation; I review the full diff every day. That is how I keep six products in production without a team.
...
- 12 Docker containers: parser, analysis, report, dashboard, logic checker, bot, API, admin panel, landing page, VPN sidecar, auto-restart, monitoring; FastAPI, CI/CD, YooKassa billing
- A companion monitor polls the API, the Telegram bot and every public site I run every 15 minutes (keyword check on the page) and alerts on failure
...
Under the hood: OpenAI Realtime as the dialogue engine, Twilio for telephony, FastAPI, a persistent queue, signed webhooks with retries. The v1.2 interface specification went through two passes: an external review (24 fixes, 3 critical) and an FPF pass (First Principles Framework, an open specification-hardening methodology; 12 critical gaps closed). Five contradictions resolved with TRIZ. Dependencies with verified licenses, CC0 backgrounds. Logs contain neither phone numbers nor conversation text.
...
- Full (non-dry-run) run over **16 weekly cutoffs**, dry-run mode, resume from any cutoff after a failure
...
### FABLE - room measurement from laser scans (Scan-to-BIM) *(private repo)*
Upload a point cloud of a room - two minutes later the measurement package is ready: IFC model for Archicad/Revit, DXF floor plan, plane-deviation maps for every wall and floor (25×25 mm grid), rough finishing calculation (plaster, reveals, screed) with quantities and a PDF estimate. Areas are easy to measure - and easy to measure wrong; a scan measures the whole surface, not three tape-measure points.
...
### [Shtab](https://protokolov.net) ("site HQ meeting") - digitizing construction site meetings
...
### [Loomio AI](https://loomio.teamplan.ru) - three LLM experts inside a self-hosted Loomio
My add-on over a self-hosted open-source [Loomio](https://www.loomio.com) instance: routing between Claude, Gemini and GPT, a reply queue in the thread, graceful degradation when a provider fails. Built and used to develop an extended methodology for construction schedule analysis in a closed group of ~100 invited scheduling experts.
...
Maintained under GRACE 4 with autonomous AFK sessions; reviewed system-wide with FPF.
...
### Risk Graph (prototype) - AI-first construction risk management *(private repo)*
...
## AI agents for clients (pilots and demos)

11 custom-built LLM agents automating sales, consulting and customer support; public demo versions exist, the live ones run on the clients' side. Most have RAG over the client's documents and integrations (Google Sheets, Zoom meeting scheduling, Bitrix24).
...
### [GRACE Framework](https://github.com/osovv/grace-marketplace) - Agent Skills for contract-driven AI development
...
GRACE is authored by @osovv. My contribution: upstream - [PR #26](https://github.com/osovv/grace-marketplace/pull/26) grace-afk, an autonomous mode with a budget and Telegram escalation (under review); in my fork - Hardening Pass 1 (anti-rationalization checklists, evidence-driven verification, knowledge graph integrity checks). In August 2026 I prepared a proposal on revertible effects: EFFECTS and REVERT fields in the module contract, an automatically assembled project effect inventory. Article [RU](https://github.com/Baho73/claudebar/blob/main/docs/grace-proposal-revertible-effects.md) and [EN](https://github.com/Baho73/claudebar/blob/main/docs/grace-proposal-revertible-effects.en.md), field-tested on claudebar during the GRACE 3 → 4 migration; the upstream PR is the next step.

### [claudebar](https://github.com/Baho73/claudebar) - a panel for parallel agent sessions
Rust + Win32, always on top, a switcher between open Claude Code sessions and editors. v0.4.1 (release, June 2026) + main (August 2026): inbox indicator with source icons (Gmail, Yandex, Mail.ru, Telegram, MAX) - the project row shows what arrived and opens the message on click. "Handled" is set by whoever read it, not by hand. Managed under GRACE 4 with revertible effects in contracts.

### MCP servers - infrastructure for AI agents
A family of Model Context Protocol servers through which my agents reach messengers, email, documents and hardware. 7 servers of my own: mail-mcp, mcp-gdocs, mcp-server-matrix, mcp-server-max, music, mech, planpulse-mcp; contribution to others: mcp-telegram (PR #49 merged). I use them in my own work; some are published.
...
- **[mcp-server-max](https://github.com/Baho73/mcp-server-max)** - MAX messenger. Plus a fork of [renosaza/max-mcp](https://github.com/renosaza/max-mcp): recovery after a client update, session fingerprint, attachment download (upstream PR is the next step).
...
- **mcp-telegram** (upstream [mcp-telegram/mcp-telegram](https://github.com/mcp-telegram/mcp-telegram)) - my contribution merged upstream: a shared-daemon (serve) mode so several MCP clients share one session ([PR #49](https://github.com/mcp-telegram/mcp-telegram/pull/49)). My fork adds GRACE governance and a systemd deployment guide.
...
### Weld Seam Detection (pilot on a production line) | [Demo](https://youtu.be/ie_D0QS-dDo)
...
### [AeroflotSeg](https://github.com/Baho73/AeroflotSeg) (hackathon)
...
### [rosreestr2coord](https://github.com/rendrom/rosreestr2coord) (by rendrom, 200+ stars) - coordinates by cadastral number
My PRs #111-#114: restored Area export, batch/export fix for the new nspd.gov.ru API, cadastral number validation, NullHandler logging (awaiting maintainer review).
...
### AudioStend (research bench) - real-time audio recognition with semantic tagging *(private repo)*
...
### EcoAuth (TG_Auth, internal infrastructure) - centralized Telegram authentication *(private repo)*
...
What is in this set:
- **11 public repositories**, six of them full cases with measured numbers. Automated tests in nine of eleven; seven READMEs state the count, 370+ in total.
...
- **My own mistakes as material.** annual-report-qa shows how a metric scored 30 out of 30 and why the honest number is 17 out of 30. vibecheck - four places where I was wrong and exactly what caught it.
...
- Case author and reviewer in educational AI projects (under NDA). Loomio AI is used for methodology work in a closed group of ~100 invited scheduling experts. idea-collection holds a 152-card shelf for course authors.
...
### [test-07-11](https://github.com/Baho73/test-07-11) - mentor for an AI engineering course
...
### [file-stats-service](https://github.com/Baho73/file-stats-service) - digit statistics over files from an external API
Downloads a file catalog from an external API under a rate limit and computes digit 0-9 statistics; FastAPI + httpx + SQLite, designed under GRACE.
